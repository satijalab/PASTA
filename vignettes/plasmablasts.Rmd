...
```
# Downloading data 
The data for this vignette can be found on the following site: 
The files include: 
...
#add meta data 
meta_data <- read.csv(file = paste0(dir, "Science2020_Arunalacham_RNA/meta_data.csv"), row.names = 1 )
...
DimPlot(pbmc, reduction="ref.umap", label=TRUE, cols = cols )
```
...
* A fragment file (produced from the aligned BAM file, we recommend using (sinto)[https://timoast.github.io/sinto/basic_usage.html])
